 ...
 * HTTP协议具有如下特点：
 *  1. 支持客户端/服务器模式
 *  2. 简单快速，客户端向服务器请求服务时，只需要发送请求方法和路径即可，常见的请求方法包括GET,POST等，每种方法规定了客户端与服务器联系
 *  的不同类型，由于HTTP协议简单，HTTP服务器程序规模小，通过速度快。
 *  3. 灵活：HTTP允许传输任意类型的数据对象，正是传输的类型由Content-Type加以标记。
 *  4. 无链接：限制每次链接只处理一个请求，服务器处理完客户端请求并收到客户端的应答后即断开链接，采用此种方式可以节省传输时间。当然也只是以
 *   短链接的处理方式，对于长链接 ，HTTP/1.1 每次链接是可以处理多个请求的，但是请求的顺序执行，因此更加准确的说法是链接在同一时刻只能处理
 *   一个请求。
 *  5. 无状态，指协议对于事务的处理没有记忆能力，意味着如果后续处理需要前面的信息，则必须重传，这样可能导致每次链接传递的数据量增大。
 *
